...
$$
e^T\phi(x_{new}) = \frac{1}{\sqrt{\lambda}}u^T\phi\phi^*(x_{new}) = \frac{1}{\sqrt{\lambda}}u^T\phi (\phi^*{x_{new}}- n^{-1}\phi^{T*}1_N) = \frac{1}{\sqrt{\lambda}}u^T(I-C)(\phi\phi(x_{new}) - n^{-1}\phi\phi^T1_n) = \frac{1}{\sqrt(\lambda)}(I-C)u^T(\kappa(x_1,x_{new}),...,\kappa(x_n,x_{new}))^T - n^{-1}k1_n)
$$
Where we have used the following formulas 
$$
\phi(x_new) = \phi(x_{new}) - n^{-1} \phi^T1_N
$$
